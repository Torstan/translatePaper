...
class LayoutStageTests(unittest.TestCase):
    def test_arrangement_preserves_text_and_coverage_across_columns_and_visual_barrier(self):
        ...
        self.assertEqual(render_plan.validate_plan_coverage(2, blocks, plan), [])
        self.assertEqual(render_plan.validate_plan_layout(plan, plan.page_size), [])
        ...
        for document in ("wait-free", "bert"):
            for fixture in iter_render_plan_fixtures(root, document):
                with self.subTest(document=document, page=fixture.page_num):
                    ...
                    pipeline.normalize_vector_text_layout(plan, plan.page_size)
                    once = render_plan.render_plan_json_dumps(plan)
                    pipeline.normalize_vector_text_layout(plan, plan.page_size)
                    self.assertEqual(render_plan.render_plan_json_dumps(plan), once)
                    ...
                    self.assertEqual(render_plan.validate_plan_coverage(fixture.page_num, fixture.blocks, plan), [])
